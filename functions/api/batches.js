export async function onRequest(context) {
  ...
  try {
    ...
    // GET — list all batches with coach and court details (joined)
    if (request.method === "GET") {
      const res = await fetch(
        `${SUPABASE_URL}/rest/v1/batches?select=*,coaches(name,mobile),courts(name)&order=id.desc`,
        { headers: authHeaders }
      );
    ...
    if (request.method === "POST") {
      ...
      if (body.action === "update") {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/batches?id=eq.${body.id}`, {
          method: "PATCH",
