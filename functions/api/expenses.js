export async function onRequest(context) {
  ...
  try {
    ...
    // GET — list all expenses with date filters
    if (request.method === "GET") {
      ...
      let queryUrl = `${SUPABASE_URL}/rest/v1/expenses?select=*&order=expense_date.desc,id.desc`;

      if (startDate) {
        queryUrl += `&expense_date=gte.${startDate}`;
      }
      if (endDate) {
        queryUrl += `&expense_date=lte.${endDate}`;
      }
      if (category) {
    ...
    if (request.method === "POST") {
      ...
      if (body.action === "update") {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/expenses?id=eq.${body.id}`, {
          method: "PATCH",
